...
```bash
git clone https://github.com/BhavWadhwaHub/StudentAI-matcher.git
cd StudentAI-matcher
```
...
> **Windows note:** If you have multiple Python versions installed, the `py` launcher may default to Python 3.13, which is incompatible with `numpy==1.26.4`. `start-servers.bat` automatically prefers the `python` command (typically 3.11) to avoid this. If running Flask manually, use `python app.py` rather than `py app.py`.
