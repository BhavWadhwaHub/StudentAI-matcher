...
def get_matches_for_student():
    ...
    try:
        ...
        student_name = (student_data.get('name', '') or '').strip()
        ...
        # BUG FIX #8: exact dict lookup fails on casing differences.
        # Mirror the case-insensitive fallback from /api/match.
        matches = results.get(student_name, [])
        if not matches and student_name:
            for key in results:
                try:
                    if key and str(key).strip().lower() == student_name.lower():
                        matches = results[key]
                        student_name = str(key).strip()  # use canonical name
                        break
                except (TypeError, AttributeError):
                    continue
