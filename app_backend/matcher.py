...
def _weighted_similarity_matching(df):
    ...
    try:
        ...
        for i, student1_name in enumerate(student_names_list):
            ...
            # BUG FIX #4: shuffle only within equal-score groups, not the whole list.
            if ENABLE_TIE_BREAKING and len(top_matches) > 1:
                from itertools import groupby
                shuffled = []
                for _, grp in groupby(top_matches, key=lambda x: x[1]):
                    group_list = list(grp)
                    random.shuffle(group_list)
                    shuffled.extend(group_list)
                top_matches = shuffled
...
def _normalize_scores(student_matches, student_data=None, df_or_dict=None):
    ...
    if len(scores) == 1:
        # BUG FIX #7: returning 100% for a single match regardless of actual similarity
        # is misleading (e.g. a 12% raw match would show as "100% compatible").
        # Return the actual raw score so the display is honest.
        return [(student_matches[0][0], round(scores[0], SIMILARITY_SCORE_PRECISION), student_matches[0][2])]
    ...
    if score_range == 0:
        # All scores identical — return the actual shared score unchanged.
        # (Artificially setting them all to 100 would mislead users — BUG FIX #7 extension.)
        return [(name, round(scores[0], SIMILARITY_SCORE_PRECISION), clust)
                for name, _, clust in student_matches]
    
    # Normalize scores: map [min_score, max_score] to [0, 100]
    # Formula: normalized = ((score - min_score) / (max_score - min_score)) * 100
    # NOTE: raw scores already include diversity bonus from calculate_weighted_similarity,
    # so do NOT add it again here — that would double-count it (BUG FIX #2).
    normalized_matches = []
    for match_name, score, is_same_cluster in student_matches:
        normalized_score = round(((score - min_score) / score_range) * 100.0, SIMILARITY_SCORE_PRECISION)
...
def preprocess_data(df):
    ...
    try:
        ...
        # BUG FIX #3: 'cuisine' was here but it is a MULTI-VALUE field (students pick several
        # cuisines), so it must be multi-label encoded below.  'language' is a SINGLE-choice
        # field and belongs here, not in multilabel_fields.
        onehot_fields = ['major', 'year', 'country', 'personality', 'studyStyle', 'language']
        onehot_encoded_dfs = []
        ...
        # BUG FIX #3: 'language' was here but it is a SINGLE-choice field (moved to onehot_fields).
        # 'cuisine' is multi-value and belongs here.
        multilabel_fields = ['cuisine', 'interests', 'movies']
...
def find_best_k(df, min_k=2, max_k=12):
    ...
    try:
        ...
        for i in range(1, len(inertias)):
            # Improvement in inertia reduction going from k_values[i-1] to k_values[i]
            ...
            if inertias[i-1] > 0:
                ...
                if normalized_improvement > max_improvement:
                    ...
                    # BUG FIX: elbow is at k_values[i] (the k that caused the big drop),
                    # not k_values[i-1] (the k before the drop).
                    best_k = k_values[i]
        ...
        for student_name in cluster_assignments.keys():
            try:
                ...
                if len(top_matches) == 0:
                    ...
                    # Sort by similarity, take top N, then normalise so scores are
                    # on the same 0-100 scale as normal-path matches.
                    # BUG FIX #5: missing _normalize_scores call meant fallback students
                    # received raw 10-30% scores while others got normalised 0-100%.
                    fallback_matches.sort(key=lambda x: -x[1])
                    fallback_matches = fallback_matches[:TOP_MATCHES_COUNT]
                    if fallback_matches:
                        fallback_matches = _normalize_scores(fallback_matches, student_data, student_data_dict)
...
def _hybrid_clustering_matching(df):
    ...
    try:
        ...
        # BUG FIX #6: original code did O(n²) full-DataFrame row scans
        # (df[df['name'] == name].iloc[0]) and had no column normalisation, so
        # lookups could silently fail when column names differed in casing.
        # Pre-build a dict once and use it throughout.
        column_mapping = {
            'Name': 'name', 'Email': 'email', 'Major': 'major', 'Year': 'year',
            'Language': 'language', 'Country': 'country', 'Personality': 'personality',
            'Study Style': 'studyStyle', 'Cuisine': 'cuisine',
            'Interests': 'interests', 'Movies': 'movies'
        }
        df_norm = df.copy().rename(columns=column_mapping)
        ...
        for _, row in df_norm.iterrows():
            try:
                name = str(row.get('name', '')).strip()
                if name:
                    student_data_dict[name] = row.to_dict()
            except (KeyError, TypeError, AttributeError):
                continue

        dynamic_threshold = _get_dynamic_threshold(len(df_norm))

        # Generate matches using weighted similarity, but prioritise within-cluster matches
        matches = {}
        ...
        for student_name in student_names:
            ...
            if student_data is None:
                if ENABLE_DEBUG_LOGGING:
                    print(f"⚠️ Student '{student_name}' not found in pre-built dict")
                continue
            student_matches = []
            ...
            # Normalise scores to 0-100 range based on actual distribution
            student_matches = _normalize_scores(student_matches, student_data, student_data_dict)

            # Enforce diversity in matches
            if ENABLE_DIVERSITY_ENFORCEMENT:
                student_matches = enforce_diversity_in_matches(
                    student_matches, student_data, student_data_dict)

            # Sort: same cluster first, then by normalised similarity
            ...
            for match_name, similarity_score, _ in top_matches:
                try:
                    match_info = _create_detailed_match(
                        student_data_dict, student_name, match_name, similarity_score)
                    detailed_matches.append(match_info)
                except Exception as e:
                    if ENABLE_DEBUG_LOGGING:
                        print(f"⚠️ Error creating detailed match for {student_name} -> {match_name}: {e}")
